...
function Todo() {
  ...
  return (
    <div>
      ...
      <input
        style={{ marginLeft: '450px', height: '45px', width: '300px', border: '2px solid black', borderRadius: '10px' }}
        type="text"
        onChange={handleInput}
        value={todo}
      />
      <button
        style={{ backgroundColor: 'yellow', width: '100px', height: '45px', border: '2px solid green', borderRadius: '6px' }}
        onClick={handleAdd}
      >
        Add
      </button>
      ...
      <div>
        <button
          style={{ backgroundColor: 'yellow', width: '100px', height: '45px', border: '2px solid green', borderRadius: '6px', marginLeft: '500px' }}
          onClick={handlemaincheck}
        >
          {but}
        </button>
        <button
          style={{ backgroundColor: 'yellow', width: '100px', height: '45px', border: '2px solid green', borderRadius: '6px', marginLeft: '50px' }}
          onClick={showAll}
        >
          All
        </button>
          ...
          (!Completed || ele.completed) && (
            <div style={{ border: '2px solid black', margin: '10px', padding: '10px' }} key={ele.id}>
              <li style={{ listStyleType: 'none' }}>
                <input type="checkbox" checked={ele.completed} onChange={() => handleCheckbox(ele.id)} />
                <h2 style={{ color: 'green' }} className={`task ${ele.completed ? 'completed-task' : ''}`}>
                  Task: <s>{ele.completed ? ele.title : ''}</s>
                  {!ele.completed ? ele.title : ''}
                </h2>
                <h2 style={{ color: 'red' }}>Completed: {ele.completed ? 'Yes' : 'No'}</h2>
                <button style={{ marginRight: '20px', border: '2px solid green', borderRadius: '6px' }} onClick={() => handleUpdate(ele.id)}>
                  Update
                </button>
                <button style={{ marginRight: '20px', border: '2px solid green', borderRadius: '6px' }} onClick={() => handleDelete(ele.id)}>
                  Delete
                </button>
              </li>
            </div>
